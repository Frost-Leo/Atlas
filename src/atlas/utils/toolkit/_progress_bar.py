...
class ProgressBar:
    ...
    **ProgressBar: Dual-output progress bar with console display and file logging**
    ...
    Class Methods:
        ...
        - update: Advance progress and optionally update description
        - set_total: Convert indeterminate to determinate progress
        - complete: Complete the progress bar to 100% and stop
        - stop: Stop the progress bar display
        - log: Log to both console and file
    """
    ...
    def _create_columns(self) -> List[ProgressColumn]:
        ...
        **_create_columns: Build list of progress bar columns based on configuration**
        ...
        columns.append(SpinnerColumn()) if self.params.show_spinner else None
        columns.append(TaskProgressColumn()) if self.params.show_percentage else None
    ...
    def _log_progress(self) -> None:
        """
        **_log_progress: Randomly decide whether to log current progress**

        This method uses a random sampling approach to decide whether to log
        the current progress, avoiding log spam while still capturing a
        representative sample of progress updates.
        """
        ...
        if (percentage == 0 or percentage == 100 or
                (percentage >= 25 and self._last_logged_progress is not None and self._last_logged_progress < 25) or
                (percentage >= 50 and self._last_logged_progress is not None and self._last_logged_progress < 50) or
                (percentage >= 75 and self._last_logged_progress is not None and self._last_logged_progress < 75)):
            self._logger.info(
        ...
        progress_delta = percentage - self._last_logged_progress if self._last_logged_progress is not None else 0
        sample_probability = min(0.3, 0.05 + progress_delta * 0.01)
    ...
    def start(self) -> None:
        ...
        **start: Initialize and display the progress bar**
        ...
        Example:
            >>> progress = ProgressBar(params)
            >>> progress.start()
            >>> # Progress bar is now visible in console
        """
    ...
    def update(self, advance: float = 1, description: Optional[str] = None) -> None:
        ...
        **update: Advance the progress bar and optionally update description**
        ...
        optionally changing the description text. Randomly logs progress to file.
        ...
        Note:
            ...
            - Randomly logs progress to avoid log spam
            - Won't advance beyond total steps for determinate progress

        Example:
            >>> progress.update(5)  # Advance by 5 steps
    ...
    def set_total(self, total: int) -> None:
        ...
        **set_total: Convert indeterminate progress to determinate**
        ...
        Example:
            >>> progress.start()  # Starts with spinner (indeterminate)
            >>> file_count = count_files()
            >>> progress.set_total(file_count)  # Now shows percentage bar
        """
        if self.progress is None or self.task_id is None:
            return
    ...
    def complete(self) -> None:
        ...
        **complete: Complete the progress bar and stop display**
        ...
        Example:
            >>> progress.complete()
            >>> # Progress bar shows 100% complete (if determinate)
            >>> # Console returns to normal state
        """
    ...
    def stop(self, completed: bool = False) -> None:
        ...
        **stop: Stop the progress bar display**
        ...
        Example:
            >>> progress.stop()  # Just stop at current progress
            >>> progress.stop(completed=True)  # Complete to 100% and stop
        """
        if self.progress is not None:
            task = self.progress.tasks[self.task_id]
